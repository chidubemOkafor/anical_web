...
const YourAnime = () => {
    ...
    useEffect(() => {
        const getAnime = async() => {
            try {
                const response = await axios.get('http://localhost:8000/api/v1/getAllTrackingAnime',credentials)
    ...
    <div className='grid-cols-3 grid gap-y-10'>
        ...
        <div className='h-[25em] w-[18em] rounded-md bg-gray-100 flex items-center justify-center'>
            <MdAdd className='size-[10em] text-gray-400' onClick = {() => setShowSearch(true) }/>
        </div>
    </div>
  )
}
